(function () {
  ...
  async function loadTypes() {
    ...
    const { data, error } = await client.rpc('listar_classificacoes_categorias', { p_campanha_id: String(rpgId()) });
    if (error) {
    ...
    form.addEventListener('submit', () => {
      ...
      const finish = async () => {
        ...
        if (!categoryId && typeof CATEGORIES !== 'undefined') {
          categoryId = CATEGORIES.find(item => !submission.beforeIds.has(String(item.id)))?.id || null;
        }
        if (!categoryId) {
        ...
        try {
          ...
          if (typeof showPageMessage === 'function') showPageMessage('A categoria foi salva, mas a classificação não pôde ser atualizada. Verifique a migration do Supabase e tente novamente.');
        }
      };
