(function () {
  ...
  const campaignId = () => typeof getSelectedRpg === 'function'
    ? getSelectedRpg()
  ...
  async function load() {
    try {
      const { data, error } = await client().rpc('listar_classificacoes_categorias', { p_campanha_id: String(campaignId()) });
      if (error) throw error;
      classifications = (data || []).map(row => ({ id: String(row.id), nome: row.nome, icone: row.icone || '📁', padrao: Boolean(row.padrao) }));
  ...
  function render() {
    ...
    if (!manager) {
      ...
      manager.id = 'categoryClassificationManager';
      manager.className = 'classification-manager';
      panel.after(manager);
    }
  ...
  function editClassification(id = '') {
    ...
    dialog.className = 'classification-editor-modal';
    ...
    dialog.querySelector('form').onsubmit = async event => {
      ...
      const nome = dialog.querySelector('#classificationName').value.trim();
      const icone = dialog.querySelector('#classificationIcon').value.trim() || '📁';
      if (!nome) return;
      try {
        const params = current
          ? { p_campanha_id: String(campaignId()), p_token: token(), p_classificacao_id: current.id, p_nome: nome, p_icone: icone }
          : { p_campanha_id: String(campaignId()), p_token: token(), p_nome: nome, p_icone: icone };
        const { error } = await client().rpc(current ? 'editar_classificacao_categoria' : 'criar_classificacao_categoria', params);
        if (error) throw error;
        dialog.close();
        await load();
  ...
  async function removeClassification(id) {
    const current = classifications.find(item => item.id === String(id));
    if (!current || current.padrao) return;
    if (!confirm(`Excluir a classificação "${current.nome}"? As categorias serão movidas para Personagens.`)) return;
    try {
      const { error } = await client().rpc('excluir_classificacao_categoria', { p_campanha_id: String(campaignId()), p_token: token(), p_classificacao_id: current.id });
      if (error) throw error;
      selected = 'all';
      await load();
      if (typeof loadCategories === 'function') await loadCategories(campaignId(), token());
    } catch (error) { console.error(error); alert(error.message || 'Não foi possível excluir a classificação.'); }
  }

  function categoryClassification(id) {
    const category = (typeof CATEGORIES !== 'undefined' ? CATEGORIES : []).find(item => String(item.id) === String(id));
    return String(category?.classificationId || DEFAULT_ID);
  }
  ...
  function applyFilter() {
    document.querySelectorAll('#categoryGrid .category-item').forEach(item => {
      ...
      if (!button) return;
      item.style.display = selected === 'all' || categoryClassification(button.dataset.category) === selected ? '' : 'none';
    });
  }

  function renderFormField() {
    const form = document.getElementById('categoryForm');
    if (!form || document.getElementById('categoryClassification')) return;
    ...
    form.addEventListener('submit', captureCategoryClassification, true);
  }
  ...
  function captureCategoryClassification() {
    ...
    window.__pendingCategoryClassification = {
      id: typeof editingCategoryId !== 'undefined' ? editingCategoryId : null,
      classificationId: document.getElementById('categoryClassification')?.value || DEFAULT_ID,
      before,
      retries: 0
    };
  ...
  async function savePendingCategoryClassification() {
    const pending = window.__pendingCategoryClassification;
    if (!pending) return;
    let id = pending.id;
    if (!id && typeof CATEGORIES !== 'undefined') id = CATEGORIES.find(item => !pending.before.has(String(item.id)))?.id || null;
    if (!id) {
      if (pending.retries++ < 30) return setTimeout(savePendingCategoryClassification, 200);
      window.__pendingCategoryClassification = null;
      return;
    }
    window.__pendingCategoryClassification = null;
    const classificationId = pending.classificationId === DEFAULT_ID
      ? classifications.find(item => item.padrao)?.id
      : pending.classificationId;
    if (!classificationId || !token()) return;
    try {
      const { error } = await client().rpc('classificar_categoria_por_classificacao', {
        p_campanha_id: String(campaignId()),
        p_categoria_id: String(id),
        p_token: token(),
        p_classificacao_id: String(classificationId)
      });
      if (error) throw error;
      if (typeof loadCategories === 'function') await loadCategories(campaignId(), token());
      applyFilter();
  ...
  function watchGrid() {
    ...
    if (!grid || grid.dataset.classificationObserver) return;
    grid.dataset.classificationObserver = 'true';
  ...
  function setup() {
    ...
    load();
    watchGrid();
    const observer = new MutationObserver(() => { render(); renderFormField(); watchGrid(); applyFilter(); });
