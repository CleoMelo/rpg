(function () {
  ...
  function styles() {
    ...
    style.textContent = `
      ...
      .category-type-filter{min-width:190px}.category-type-filter select{width:100%}
  ...
  function ensureFilter() {
    ...
    field.innerHTML = `<label for="categoryTypeFilter">Classificação</label><select id="categoryTypeFilter"><option value="all">Todos os tipos</option>${TYPES.map(t => `<option value="${t.value}">${t.icon} ${escapeHtml(t.label)}</option>`).join('')}</select>`;
    ...
    field.querySelector('select').addEventListener('change', event => { filterValue = event.currentTarget.value; applyFilter(); });
  }
    ...
    form.addEventListener('submit', () => {
      ...
      const finish = async () => {
        ...
        try {
          ...
          if (typeof showPageMessage === 'function') showPageMessage('A categoria foi salva, mas a classificação não pôde ser atualizada. Execute a migration do Supabase e tente novamente.');
        }
      };
  ...
  function setup() {
    ...
    if (grid && !gridObserver) {
      gridObserver = new MutationObserver(() => { decorate(); updateSelectedLabels(); });
