(function () {
  ...
  async function loadTypes() {
    ...
    const { data, error } = await client.from('categorias').select('id,tipo').eq('campanha_id', String(rpgId()));
    if (error) { console.error('Classificação das categorias:', error); return; }
    categoryTypes = new Map((data || []).map(row => [String(row.id), typeMap.has(String(row.tipo)) ? String(row.tipo) : 'personagem']));
    if (typeof CATEGORIES !== 'undefined') CATEGORIES.forEach(category => { if (!categoryTypes.has(String(category.id))) categoryTypes.set(String(category.id), 'personagem'); });
    decorate();
    updateSelectedLabels();
  }

  function loadLocalTypes() {
    if (typeof isDefaultRpg !== 'function' || !isDefaultRpg(rpgId())) return;
    try {
      const saved = JSON.parse(localStorage.getItem(`categoryTypes:${rpgId()}`) || '{}');
      Object.entries(saved).forEach(([id, type]) => { if (typeMap.has(String(type))) categoryTypes.set(String(id), String(type)); });
  ...
  async function saveType(categoryId, type) {
    ...
    const client = getSupabaseClient();
    const { data, error } = await client.rpc('classificar_categoria', { p_campanha_id: String(rpgId()), p_categoria_id: String(categoryId), p_token: getMasterToken(rpgId()), p_tipo: normalized });
  ...
  function interceptSubmit() {
    ...
    form.addEventListener('submit', () => {
      pendingSubmission = { categoryId: typeof editingCategoryId !== 'undefined' ? editingCategoryId : null, type: document.getElementById('categoryType')?.value || 'personagem', beforeIds: typeof CATEGORIES !== 'undefined' ? new Set(CATEGORIES.map(item => String(item.id))) : new Set() };
      ...
      const finish = async () => {
        ...
        pendingSubmission = null;
        try { await saveType(categoryId, submission.type); decorate(); updateSelectedLabels(); }
        catch (error) { console.error('Não foi possível salvar a classificação:', error); if (typeof showPageMessage === 'function') showPageMessage('A categoria foi salva, mas a classificação não pôde ser atualizada. Execute a migration do Supabase e tente novamente.'); }
      };
